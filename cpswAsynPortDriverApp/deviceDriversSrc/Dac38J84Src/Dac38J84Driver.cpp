...
Dac38J84Driver::Dac38J84Driver(const char *portName, Path p, int nelms)
                 :cpswAsynDriver(portName, p->findByName("Dac38J84"), nelms, NUM_DAC38J84_PARAMS)
{
...
asynStatus Dac38J84Driver::InitDAC_JESD()
{
   asynStatus status = asynSuccess;
/* Fill in command logic here */
   printf("Command InitDAC_JESD not yet implemented!\n");
...
extern "C" int Dac38J84Create(const char *portName, const char *path)
{
  ...
  new Dac38J84Driver(portName, p, c->getNelms());
  }
  return(asynSuccess);
}
